...
void ShapeData::Init(int shape, int frame, bool shouldreset) {
    ...
    if (m_originalTexture != nullptr) {
        ...
        m_shapePointerTexture->AssignImage(
            g_shapeTable[m_pointerShape][m_pointerFrame]
        ...
        if (m_topTexture == nullptr) {
            m_topTexture =
                std::make_unique<ModTexture>(m_originalTexture->m_Image);
        }
...
void ShapeData::SetupDrawTypes() {
    ...
    m_Dims =
        Vector3{objectData->m_width, objectData->m_height, objectData->m_depth};

    if (m_drawType == ShapeDrawType::OBJECT_DRAW_BILLBOARD) {
        m_Dims = Vector3{float(m_originalTexture->width) / 8.0f,
                         float(m_originalTexture->height) / 8.0f, 1};
    } else if (m_drawType == ShapeDrawType::OBJECT_DRAW_FLAT) {
        m_Dims = Vector3{float(m_originalTexture->width) / 8.0f, 0,
                         float(m_originalTexture->height) / 8.0f};
    } else {
        m_Dims = Vector3{objectData->m_width, objectData->m_height,
                         objectData->m_depth};
    }
    ...
    UpdateMeshBuffer(flatMesh, 0, flatMesh.vertices,
                     sizeof(float) * flatMesh.vertexCount * 3, 0);

    m_flatModel = LoadModelFromMesh(flatMesh);
    SetMaterialTexture(&m_flatModel.materials[0], MATERIAL_MAP_DIFFUSE,
                       m_originalTexture->m_Texture);

    //  CUBOID DRAWING

    //  Very similar to flat drawing, we need to build six custom meshes to
    //  represent the sides.
    ...
    UpdateMeshBuffer(cuboidMesh, 0, cuboidMesh.vertices,
                     sizeof(float) * cuboidMesh.vertexCount * 3, 0);

    //  Now that we have made the cuboid mesh, split those vertices and UV
    //  coordinates into separate meshes for each side.
    ...
    m_faceCenterPoints[CuboidSides::CUBOID_BOTTOM] = Vector3{
        (bottomMesh.vertices[0] + bottomMesh.vertices[3] +
         bottomMesh.vertices[6] + bottomMesh.vertices[9]) /
            4,
        (bottomMesh.vertices[1] + bottomMesh.vertices[4] +
         bottomMesh.vertices[7] + bottomMesh.vertices[10]) /
            4,
        (bottomMesh.vertices[2] + bottomMesh.vertices[5] +
         bottomMesh.vertices[8] + bottomMesh.vertices[11]) /
            4,
    };

    UpdateMeshBuffer(bottomMesh, 0, bottomMesh.vertices,
                     sizeof(float) * bottomMesh.vertexCount * 3, 0);
    m_cuboidModels[static_cast<int>(CuboidSides::CUBOID_BOTTOM)] =
        LoadModelFromMesh(bottomMesh);
    ...
    m_faceCenterPoints[CuboidSides::CUBOID_TOP] = Vector3{
        (topMesh.vertices[0] + topMesh.vertices[3] + topMesh.vertices[6] +
         topMesh.vertices[9]) /
            4,
        (topMesh.vertices[1] + topMesh.vertices[4] + topMesh.vertices[7] +
         topMesh.vertices[10]) /
            4,
        (topMesh.vertices[2] + topMesh.vertices[5] + topMesh.vertices[8] +
         topMesh.vertices[11]) /
            4,
    };

    UpdateMeshBuffer(topMesh, 0, topMesh.vertices,
                     sizeof(float) * topMesh.vertexCount * 3, 0);
    m_cuboidModels[static_cast<int>(CuboidSides::CUBOID_TOP)] =
        LoadModelFromMesh(topMesh);
    ...
    m_faceCenterPoints[CuboidSides::CUBOID_LEFT] = Vector3{
        (leftMesh.vertices[0] + leftMesh.vertices[3] + leftMesh.vertices[6] +
         leftMesh.vertices[9]) /
            4,
        (leftMesh.vertices[1] + leftMesh.vertices[4] + leftMesh.vertices[7] +
         leftMesh.vertices[10]) /
            4,
        (leftMesh.vertices[2] + leftMesh.vertices[5] + leftMesh.vertices[8] +
         leftMesh.vertices[11]) /
            4,
    };

    UpdateMeshBuffer(leftMesh, 0, leftMesh.vertices,
                     sizeof(float) * leftMesh.vertexCount * 3, 0);
    m_cuboidModels[static_cast<int>(CuboidSides::CUBOID_LEFT)] =
        LoadModelFromMesh(leftMesh);
    ...
    m_faceCenterPoints[CuboidSides::CUBOID_RIGHT] = Vector3{
        (rightMesh.vertices[0] + rightMesh.vertices[3] + rightMesh.vertices[6] +
         rightMesh.vertices[9]) /
            4,
        (rightMesh.vertices[1] + rightMesh.vertices[4] + rightMesh.vertices[7] +
         rightMesh.vertices[10]) /
            4,
        (rightMesh.vertices[2] + rightMesh.vertices[5] + rightMesh.vertices[8] +
         rightMesh.vertices[11]) /
            4,
    };

    UpdateMeshBuffer(rightMesh, 0, rightMesh.vertices,
                     sizeof(float) * rightMesh.vertexCount * 3, 0);
    m_cuboidModels[static_cast<int>(CuboidSides::CUBOID_RIGHT)] =
        LoadModelFromMesh(rightMesh);
    ...
    m_faceCenterPoints[CuboidSides::CUBOID_FRONT] = Vector3{
        (frontMesh.vertices[0] + frontMesh.vertices[3] + frontMesh.vertices[6] +
         frontMesh.vertices[9]) /
            4,
        (frontMesh.vertices[1] + frontMesh.vertices[4] + frontMesh.vertices[7] +
         frontMesh.vertices[10]) /
            4,
        (frontMesh.vertices[2] + frontMesh.vertices[5] + frontMesh.vertices[8] +
         frontMesh.vertices[11]) /
            4,
    };

    UpdateMeshBuffer(frontMesh, 0, frontMesh.vertices,
                     sizeof(float) * frontMesh.vertexCount * 3, 0);
    m_cuboidModels[static_cast<int>(CuboidSides::CUBOID_FRONT)] =
        LoadModelFromMesh(frontMesh);
    ...
    m_faceCenterPoints[CuboidSides::CUBOID_BACK] = Vector3{
        (backMesh.vertices[0] + backMesh.vertices[3] + backMesh.vertices[6] +
         backMesh.vertices[9]) /
            4,
        (backMesh.vertices[1] + backMesh.vertices[4] + backMesh.vertices[7] +
         backMesh.vertices[10]) /
            4,
        (backMesh.vertices[2] + backMesh.vertices[5] + backMesh.vertices[8] +
         backMesh.vertices[11]) /
            4,
    };

    UpdateMeshBuffer(backMesh, 0, backMesh.vertices,
                     sizeof(float) * backMesh.vertexCount * 3, 0);
    m_cuboidModels[static_cast<int>(CuboidSides::CUBOID_BACK)] =
        LoadModelFromMesh(backMesh);
...
void ShapeData::FixupTextures() {
    ...
    //  Top face
    m_topTexture->m_Image = ImageFromImage(
        textureToUse->m_Image,
        Rectangle{float(m_topTextureOffsetX), float(m_topTextureOffsetY),
                  float(m_topTextureWidth), float(m_topTextureHeight)});
    m_topTexture->UpdateTexture();

    //  Front face
    m_frontTexture->m_Image = ImageFromImage(
        textureToUse->m_Image,
        Rectangle{float(m_frontTextureOffsetX), float(m_frontTextureOffsetY),
                  float(textureToUse->m_Image.width - m_frontTextureOffsetX),
                  float(textureToUse->m_Image.height - m_frontTextureOffsetY)});
    ...
    //  Right face
    m_rightTexture->m_Image = ImageFromImage(
        textureToUse->m_Image,
        Rectangle{float(m_rightTextureOffsetX), float(m_rightTextureOffsetY),
                  float(textureToUse->m_Image.width - m_rightTextureOffsetX),
                  float(textureToUse->m_Image.height - m_rightTextureOffsetY)});
...
void ShapeData::Draw(const Vector3& pos, float angle, Color color,
    ...
    case ShapeDrawType::OBJECT_DRAW_CUBOID: {
        ...
        float   leftDistance = Vector3Distance(
            g_camera.position,
            Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_LEFT], pos));
        float rightDistance = Vector3Distance(
            g_camera.position,
            Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_RIGHT], pos));
        float topDistance = Vector3Distance(
            g_camera.position,
            Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_TOP], pos));
        float bottomDistance = Vector3Distance(
            g_camera.position,
            Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_BOTTOM], pos));
        float frontDistance = Vector3Distance(
            g_camera.position,
            Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_FRONT], pos));
        float backDistance = Vector3Distance(
            g_camera.position,
            Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_BACK], pos));

        vector<pair<CuboidSides, float>> distances = {
            make_pair(CuboidSides::CUBOID_LEFT, leftDistance),
            make_pair(CuboidSides::CUBOID_RIGHT, rightDistance),
            make_pair(CuboidSides::CUBOID_TOP, topDistance),
            make_pair(CuboidSides::CUBOID_BOTTOM, bottomDistance),
            make_pair(CuboidSides::CUBOID_FRONT, frontDistance),
            make_pair(CuboidSides::CUBOID_BACK, backDistance)};

        sort(distances.begin(), distances.end(),
             [](const pair<CuboidSides, float>& a,
                const pair<CuboidSides, float>& b) {
                 return a.second > b.second;
             });

        for (int i = 0; i < distances.size(); ++i) {
            if (m_sideTextures[static_cast<int>(distances[i].first)] !=
                CuboidTexture::CUBOID_DONT_DRAW) {
                DrawModelEx(
                    m_cuboidModels[static_cast<int>(distances[i].first)],
                    finalPos, Vector3{0, 1, 0}, angle, cuboidScaling, WHITE);
            }
        }
    ...
    case ShapeDrawType::OBJECT_DRAW_BILLBOARD: {
        ...
        BeginShaderMode(g_alphaDiscard);
        DrawBillboardPro(
            g_camera, m_originalTexture->m_Texture,
            Rectangle{0, 0, float(m_originalTexture->m_Texture.width),
                      float(m_originalTexture->m_Texture.height)},
            finalPos, Vector3{0, 1, 0}, Vector2{m_Dims.x, m_Dims.y},
            Vector2{0, 0}, -45, color);
        EndShaderMode();
        break;
    }
...
void ShapeData::SetTextureForMeshFromSideData(CuboidSides side) {
    ...
    case CuboidTexture::CUBOID_DRAW_TOP: {
        SetMaterialTexture(&m_cuboidModels[static_cast<int>(side)].materials[0],
                           MATERIAL_MAP_DIFFUSE, m_topTexture->m_Texture);
        break;
    }
    case CuboidTexture::CUBOID_DRAW_FRONT: {
        SetMaterialTexture(&m_cuboidModels[static_cast<int>(side)].materials[0],
                           MATERIAL_MAP_DIFFUSE, m_frontTexture->m_Texture);
        break;
    }
    case CuboidTexture::CUBOID_DRAW_RIGHT: {
        SetMaterialTexture(&m_cuboidModels[static_cast<int>(side)].materials[0],
                           MATERIAL_MAP_DIFFUSE, m_rightTexture->m_Texture);
        break;
    }
    case CuboidTexture::CUBOID_DRAW_TOP_INVERTED: {
        ...
        SetMaterialTexture(&m_cuboidModels[static_cast<int>(side)].materials[0],
                           MATERIAL_MAP_DIFFUSE, LoadTextureFromImage(image));
        break;
    }
...
void ShapeData::UpdateShapePointerTexture() {
    ...
    m_shapePointerTexture->AssignImage(
        g_shapeTable[m_pointerShape][m_pointerFrame].GetDefaultTextureImage());
}
