...
void ShapeData::SetupDrawTypes()
{
	...
	{
		m_Dims = Vector3{
			float(m_originalTexture->width) / 8.0f, float(m_originalTexture->height) / 8.0f, 1
		};
	}
	else if (m_drawType == ShapeDrawType::OBJECT_DRAW_FLAT)
	{
		m_Dims = Vector3{
			float(m_originalTexture->width) / 8.0f, 0, float(m_originalTexture->height) / 8.0f
		};
	}
	else
	{
	...
	SetMaterialTexture(
		&m_flatModel.materials[0], MATERIAL_MAP_DIFFUSE, m_originalTexture->m_Texture
	);
	...
	UpdateMeshBuffer(
		cuboidMesh, 0, cuboidMesh.vertices, sizeof(float) * cuboidMesh.vertexCount * 3, 0
	);
	...
	m_faceCenterPoints[CuboidSides::CUBOID_BOTTOM] = Vector3{
		(bottomMesh.vertices[0] + bottomMesh.vertices[3] + bottomMesh.vertices[6] +
		 bottomMesh.vertices[9]) /
			4,
		(bottomMesh.vertices[1] + bottomMesh.vertices[4] + bottomMesh.vertices[7] +
		 bottomMesh.vertices[10]) /
			4,
		(bottomMesh.vertices[2] + bottomMesh.vertices[5] + bottomMesh.vertices[8] +
		 bottomMesh.vertices[11]) /
			4,
	};

	UpdateMeshBuffer(
		bottomMesh, 0, bottomMesh.vertices, sizeof(float) * bottomMesh.vertexCount * 3, 0
	);
	...
	m_faceCenterPoints[CuboidSides::CUBOID_TOP] = Vector3{
		...
		(topMesh.vertices[1] + topMesh.vertices[4] + topMesh.vertices[7] + topMesh.vertices[10]) /
			4,
		(topMesh.vertices[2] + topMesh.vertices[5] + topMesh.vertices[8] + topMesh.vertices[11]) /
			4,
	};
	...
	m_faceCenterPoints[CuboidSides::CUBOID_LEFT] = Vector3{
		(leftMesh.vertices[0] + leftMesh.vertices[3] + leftMesh.vertices[6] + leftMesh.vertices[9]
		) / 4,
		(leftMesh.vertices[1] + leftMesh.vertices[4] + leftMesh.vertices[7] + leftMesh.vertices[10]
		) / 4,
		(leftMesh.vertices[2] + leftMesh.vertices[5] + leftMesh.vertices[8] + leftMesh.vertices[11]
		) / 4,
	};
	...
	UpdateMeshBuffer(
		rightMesh, 0, rightMesh.vertices, sizeof(float) * rightMesh.vertexCount * 3, 0
	);
	...
	UpdateMeshBuffer(
		frontMesh, 0, frontMesh.vertices, sizeof(float) * frontMesh.vertexCount * 3, 0
	);
	...
	m_faceCenterPoints[CuboidSides::CUBOID_BACK] = Vector3{
		(backMesh.vertices[0] + backMesh.vertices[3] + backMesh.vertices[6] + backMesh.vertices[9]
		) / 4,
		(backMesh.vertices[1] + backMesh.vertices[4] + backMesh.vertices[7] + backMesh.vertices[10]
		) / 4,
		(backMesh.vertices[2] + backMesh.vertices[5] + backMesh.vertices[8] + backMesh.vertices[11]
		) / 4,
	};
...
void ShapeData::FixupTextures()
{
	...
	m_topTexture->m_Image = ImageFromImage(
		...
		Rectangle{
			float(m_topTextureOffsetX),
			float(m_topTextureOffsetY),
			float(m_topTextureWidth),
			float(m_topTextureHeight)
		}
	);
	...
	m_frontTexture->m_Image = ImageFromImage(
		textureToUse->m_Image,
		Rectangle{
			float(m_frontTextureOffsetX),
			float(m_frontTextureOffsetY),
	...
	m_rightTexture->m_Image = ImageFromImage(
		textureToUse->m_Image,
		Rectangle{
			float(m_rightTextureOffsetX),
			float(m_rightTextureOffsetY),
...
void ShapeData::Draw(const Vector3& pos, float angle, Color color, Vector3 scaling)
{
	...
	case ShapeDrawType::OBJECT_DRAW_CUBOID:
	{
		...
		float   leftDistance = Vector3Distance(
            g_camera.position, Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_LEFT], pos)
        );
		float rightDistance = Vector3Distance(
			g_camera.position, Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_RIGHT], pos)
		);
		float topDistance = Vector3Distance(
			g_camera.position, Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_TOP], pos)
		);
		float bottomDistance = Vector3Distance(
			g_camera.position, Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_BOTTOM], pos)
		);
		float frontDistance = Vector3Distance(
			g_camera.position, Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_FRONT], pos)
		);
		float backDistance = Vector3Distance(
			g_camera.position, Vector3Add(m_faceCenterPoints[CuboidSides::CUBOID_BACK], pos)
		);
		...
		sort(
			distances.begin(),
			distances.end(),
			[](const pair<CuboidSides, float>& a, const pair<CuboidSides, float>& b)
	...
	case ShapeDrawType::OBJECT_DRAW_BILLBOARD:
	{
		...
		DrawBillboardPro(
			...
			Rectangle{
				0,
				0,
				float(m_originalTexture->m_Texture.width),
				float(m_originalTexture->m_Texture.height)
			},
			finalPos,
			Vector3{0, 1, 0},
			Vector2{m_Dims.x, m_Dims.y},
			Vector2{0, 0},
			-45,
			color
		);
...
void ShapeData::SetTextureForMeshFromSideData(CuboidSides side)
{
	...
	{
		SetMaterialTexture(
			&m_cuboidModels[static_cast<int>(side)].materials[0],
			MATERIAL_MAP_DIFFUSE,
			m_topTexture->m_Texture
		);
		break;
	}
