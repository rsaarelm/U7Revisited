...
class GuiElement : public Tween {
 public:
    ...
    bool  m_Hovered =
        false;  //  The mouse is over this element but is not down.  Somne
                //  elements will draw differently in this state.
    bool m_Down = false;  //  The mouse is over this element and the left mouse
                          //  button is down.  The element will draw "clicked".
    bool m_Clicked =
        false;  //  The mouse is over this element and the left mouse button has
                //  JUST been released.  This is usually what you're looking
                //  for.  The element will draw normally since this is the end
                //  of the interaction.
...
//  The TextButton is procedurally generated from a string and a box size.  The
//  colors invert when the button is clicked.
class GuiTextButton : public GuiElement {
 public:
    ...
    void  Init(int ID, int posx, int posy, int width, int height,
               ...
               Color bordercolor = Color{255, 255, 255, 255}, int group = 0,
               int active = true);
...
class GuiIconButton : public GuiElement {
 public:
    ...
    void Init(int ID, int posx, int posy, std::shared_ptr<Sprite> upbutton,
              std::shared_ptr<Sprite> downbutton = NULL,
              std::shared_ptr<Sprite> inactivebutton = NULL,
              std::string text = "", Font* font = NULL,
              Color fontcolor = (Color{255, 255, 255, 255}), int group = 0,
              int active = true, bool canbeheld = false);
...
class GuiCheckBox : public GuiElement {
 public:
    ...
    void Init(int ID, int posx, int posy, std::shared_ptr<Sprite> selectSprite,
              std::shared_ptr<Sprite> deselectSprite,
              std::shared_ptr<Sprite> hoveredSprite = nullptr,
              std::shared_ptr<Sprite> hoveredselected = nullptr,
              float scalex = 1.0f, float scaley = 1.0f,
              Color color = Color{255, 255, 255, 255}, int group = 0,
              int active = true);
...
class GuiScrollBar : public GuiElement {
 public:
    ...
    void Init(int ID, int valuerange, int posx, int posy, int width, int height,
              bool vertical, Color spurcolor = Color{128, 128, 255, 255},
              Color backgroundcolor = Color{0, 0, 0, 0}, int group = 0,
              int active = true, bool shadowed = false);

    void Init(int ID, int valuerange, int posx, int posy, int width, int height,
              bool vertical, std::shared_ptr<Sprite> activeLeft,
              ...
              std::shared_ptr<Sprite> spurInactive = nullptr, int group = 0,
              int active = true, bool shadowed = false);
...
//  player can then use backspace to delete the current text and type whatever
//  text he wishes.  No movement within the string will be allowed.  You can
//  specify a border with a color as well as a font color, but if you want a
//  background, use a panel, it's what they are for.
...
class GuiTextArea : public GuiElement {
 public:
    ...
    void  Init(int ID, Font* font, std::string text, int posx, int posy,
               int width = 0, int height = 0,
               Color color = Color{255, 255, 255, 255},
               int justified = GuiTextArea::LEFT, int group = 0,
               int active = true, bool shadowed = false);
    ...
    enum Justification { LEFT = 0, CENTERED, RIGHT };
};
...
class GuiSprite : public GuiElement {
 public:
    ...
    void Init(int ID, int posx, int posy, std::shared_ptr<Sprite> sprite,
              float scalex = 1.0f, float scaley = 1.0f,
              Color color = Color{255, 255, 255, 255}, int group = 0,
              int active = true);
