...
void Emitter2D::Draw() {
    ...
    for (node; node != m_Particles.end(); ++node) {
        ...
        float       drawx =
            (GetPos().x + temp.m_Pos.x - m_DrawOffset.x) * temp.m_Scale +
            (GetScreenWidth() / 2);
        float drawy =
            (GetPos().y + temp.m_Pos.y - m_DrawOffset.y) * temp.m_Scale +
            (GetScreenHeight() / 2);

        Color c;
        float pct = 0;
        if (temp.m_MaxAge > 0)
            pct = (float)(temp.m_Age - temp.m_Birth) /
                  (float)(temp.m_MaxAge - temp.m_Birth);
        if (pct > 1.0f)
            pct = 1.0f;
        if (pct < 0)
            pct = 0;
        c.r = (temp.m_StartColor.r * (1.0f - pct) + temp.m_EndColor.r * pct) *
              m_ColorMask.r;
        c.g = (temp.m_StartColor.g * (1.0f - pct) + temp.m_EndColor.g * pct) *
              m_ColorMask.g;
        c.b = (temp.m_StartColor.b * (1.0f - pct) + temp.m_EndColor.b * pct) *
              m_ColorMask.b;
        c.a = (temp.m_StartColor.a * (1.0f - pct) + temp.m_EndColor.a * pct) *
              m_ColorMask.a;

        m_Sprites[temp.m_Sprite]->DrawScaled(
            Rectangle{drawx, drawy, temp.m_Scale, temp.m_Scale}, Vector2{0, 0},
            temp.m_Angle, c);
    }
}
...
void Emitter2D::AddParticle(Vector2 pos, Vector2 vel, unsigned int maxage,
                            Color startcolor, Color endcolor, float rotation,
                            float scale) {
    Particle2D temp;
...
void Emitter2D::AddParticle(Particle2D particle) {
    particle.m_Sprite = m_RNG.Random(
        int(m_Sprites.size()) - 1);  // Assign a random sprite to this particle
    m_Particles.push_back(particle);
}

/*void Emitter3D::AddParticle(Vector3 pos, Vector3 vel, Vector3 acc,
             unsigned int maxage, Color startcolor, Color endcolor, float
   ...
   for(node; node != m_Particles.end(); ++node)
   {
      if (GetDrawMode() == DM_2D)
      {
         DrawImage(m_Texture,
            ...
            m_Texture->width * (*node).m_Scale.x, m_Texture->height *
(*node).m_Scale.y,
            (*node).m_StartColor, false, (*node).m_Angle);
      }
      else
      {
         DrawMesh(m_Mesh, (*node).m_Pos, m_Texture, (*node).m_StartColor,
Vector3(0, GetCameraAngle() + 45, (*node).m_Angle), (*node).m_Scale);
      }
   }
}*/
