...
class Emitter2D : public Unit2D {
 public:
    ...
    void         Stop() {
        m_Started = false;
    }  //  Stop the emitter.  It won't emit any more particles but particles
       //  already in existance will still be updated and drawn.
    void
            AddSprite(std::shared_ptr<Sprite>
                          sprite);  // Adds a sprite to the list of sprites that can be
                            // used for this emitter. Adding a particle will
                            // choose a random sprite from the list.
    void    AddParticle(Particle2D particle);
    void    AddParticle(Vector2 pos, Vector2 vel, unsigned int maxage,
                        Color startcolor = WHITE, Color endcolor = WHITE,
                        float rotation = 0, float scale = 1);
    void    SetTexture(Texture* texture) { m_Texture = texture; }
    void    SetColorMask(Color color) { m_ColorMask = color; }
    Vector2 m_DrawOffset;  // Just in case our position doesn't match our screen
                           // coordinates.
    std::vector<Particle2D> m_Particles;

 private:
    std::vector<std::shared_ptr<Sprite>> m_Sprites;
    Texture* m_Texture;  // The same texture will be used for all particles from
                         // this emitter.

    bool     m_Started;
    bool m_DieOnEmpty;  // This emitter will be destroyed when all the particles
                        // in it have died.
    RNG  m_RNG;
...
public:
   ...
   void AddParticle(Vector3 pos, Vector3 vel, Vector3 acc,
      ...
      Color endcolor = Color{255, 255, 255, 255}, float rotation = 0, Vector3
scale = Vector3(1, 1, 1));
...
enum PS_UPDATETYPES
{
   ...
   PS_OSCILLATE,        //  "Drifts" back and forth along the line of the values
given. PS_LASTUPDATETYPE
};*/
