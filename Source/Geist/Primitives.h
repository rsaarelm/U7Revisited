...
Vertex CreateVertex(float inx = 0, float iny = 0, float inz = 0, float inr = 1,
...
Vertex2D CreateVertex2D(float inx = 0, float iny = 0, float inr = 1,
                        float ing = 1, float inb = 1, float ina = 1,
                        float inu = 0, float inv = 0);
...
class ColoredString {
 public:
    ColoredString(std::string instring = "",
                  Color color = ColorFromNormalized(Vector4{1.0f, 1.0f, 1.0f,
                                                            1.0f})) {
        m_String = instring;
        m_Color = color;
    }
...
class Sprite {
 public:
    ...
    Sprite() {
        ...
        m_sourceRect.x = 0;
        m_sourceRect.y = 0;
        m_sourceRect.width = 0;
        m_sourceRect.height = 0;
    }
    Sprite(Texture* texture, float posx, float posy, float width,
           float height) {
        m_texture = texture;
        m_sourceRect.x = posx;
        m_sourceRect.y = posy;
    ...
    Sprite(Texture* texture, int posx, int posy, int width, int height) {
        m_texture = texture;
        m_sourceRect.x = float(posx);
        m_sourceRect.y = float(posy);
        m_sourceRect.width = float(width);
        m_sourceRect.height = float(height);
    }

    void Draw(float x, float y, Color tint = WHITE);
    void Draw(Vector2 pos, Color tint = WHITE);
    void DrawScaled(Rectangle dest, Vector2 origin = Vector2{0, 0},
                    float rotation = 0, Color tint = WHITE);
};

//  A "tween" is a point that moves between a start and destination point in a
//  programmable way.
class Tween : public Object {
 public:
    Tween() {};
    void Init(const std::string& data) {
        throw("Tweens can't be initialized from data (yet).");
    }
    ...
    void    Reverse();  // Swaps the pos and dest of an element, allowing it to
                        // move back and forth.
    void    PopToStart() { m_Pos = m_Start; }
    void    PopToDestination() { m_Pos = m_Dest; }
    bool    Done() { return m_Done; }

    Vector2 m_Start;  //  Need to know where the sprite started for certain
                      //  movement types
    Vector2 m_Pos;
    ...
    enum {
        MOVE_NORMAL = 0,  // Move to the target at specified speed.  Stop when
                          // we get there.
        MOVE_EASING,  //  Move to the target at specified speed, slowing down to
                      //  a stop as we arrive.
        MOVE_BOUNCE,  //  Move to the target at specified speed and
                      //  acceleration, halving acceleration and reversing
                      //  direction when we reach the target until speed = 0;
        MOVE_PARABOLIC,
        MOVE_LASTMOVE
    };
};
...
class MobileSprite : public Tween {
 public:
    MobileSprite() {};
    void Init(Sprite sprite, Vector2 start, Vector2 dest, float speed,
              float acceleration);
    void Update();
    void Draw();
    void Shutdown() {};
...
//  An animation is a group of sprites, along with the order in which they
//  should be displayed and the frame rate they should be displayed.
class Animation {
 public:
    Animation() {}
    void AddFrames(Texture* texture, int posx, int posy, int width, int height,
                   int numFrames);
    void AddFrame(std::shared_ptr<Sprite> sprite) {
        m_Frames.push_back(sprite);
    }
    void AddFrames(std::vector<std::shared_ptr<Sprite>> frames) {
        m_Frames = frames;
    }
    void AddFramesSpaced(Texture* texture, int posx, int posy, int width,
                         int height, int numFrames, int xstep);
    void AddAnim(std::string name, std::initializer_list<int> frames,
                 int frameRate, bool looping);
    void AddAnim(std::string name, std::vector<int> frames, int frameRate,
                 bool looping);
    void Update();
 ...
 private:
    ...
    struct Anim {
        ...
        Anim(std::initializer_list<int> frameOrder, int frameRate,
             bool looping) {
            m_FrameOrder = frameOrder;
            m_FrameRate = frameRate;
            m_Looping = looping;
            m_MSPerFrame = int(m_FrameOrder.size()) / 1000;
        }
        Anim(std::vector<int> frameOrder, int frameRate, bool looping) {
            m_FrameOrder = frameOrder;
            m_FrameRate = frameRate;
            m_Looping = looping;
            m_MSPerFrame = int(m_FrameOrder.size()) / 1000;
...
class ModTexture  // A texture with an associated image that can be modified
                  // after creation.
{
 public:
    ModTexture() {};
