...
Vertex CreateVertex(
	float inx = 0, float iny = 0, float inz = 0, float inr = 1, float ing = 1, float inb = 1,
	float ina = 1, float inu = 0, float inv = 0
);

Vertex CreateVertex(
	float inx = 0, float iny = 0, float inz = 0,
	Color color = ColorFromNormalized(Vector4{1, 1, 1, 1}), float inu = 0, float inv = 0
);
...
Vertex2D CreateVertex2D(
	float inx = 0, float iny = 0, float inr = 1, float ing = 1, float inb = 1, float ina = 1,
	float inu = 0, float inv = 0
);

Vertex2D CreateVertex2D(
	float inx = 0, float iny = 0, Color color = ColorFromNormalized(Vector4{1, 1, 1, 1}),
	float inu = 0, float inv = 0
);
...
public:
	ColoredString(
		std::string instring = "",
		Color       color = ColorFromNormalized(Vector4{1.0f, 1.0f, 1.0f, 1.0f})
	)
	{
		m_String = instring;
		m_Color = color;
...
public:
	...
	Sprite(Texture* texture, float posx, float posy, float width, float height)
	{
		m_texture = texture;
		m_sourceRect.x = posx;
		m_sourceRect.y = posy;
	...
	void DrawScaled(
		Rectangle dest, Vector2 origin = Vector2{0, 0}, float rotation = 0, Color tint = WHITE
	);
};
...
private:
	...
	struct Anim
	{
		...
		Anim(std::initializer_list<int> frameOrder, int frameRate, bool looping)
		{
