...
void RNG::SeedRNG(unsigned int n) {
    ...
    for (m_SeedIndex = 1; m_SeedIndex < CMATH_N; m_SeedIndex++) {
        m_SeedTable[m_SeedIndex] =
            (69069 * m_SeedTable[m_SeedIndex - 1]) & 0xffffffff;
    }
...
unsigned int RNG::Random(unsigned int n) {
    ...
    if (m_SeedIndex >= CMATH_N) {
        ...
        for (kk = 0; kk < CMATH_N - CMATH_M; kk++) {
            y = (m_SeedTable[kk] & 0x80000000) |
                (m_SeedTable[kk + 1] & 0x7fffffff);
            m_SeedTable[kk] =
                m_SeedTable[kk + CMATH_M] ^ (y >> 1) ^ mag01[y & 0x1];
        }

        for (; kk < CMATH_N - 1; kk++) {
            y = (m_SeedTable[kk] & 0x80000000) |
                (m_SeedTable[kk + 1] & 0x7fffffff);
            m_SeedTable[kk] = m_SeedTable[kk + (CMATH_M - CMATH_N)] ^ (y >> 1) ^
                              mag01[y & 0x1];
        }

        y = (m_SeedTable[CMATH_N - 1] & 0x80000000) |
            (m_SeedTable[0] & 0x7fffffff);
        m_SeedTable[CMATH_N - 1] =
            m_SeedTable[CMATH_M - 1] ^ (y >> 1) ^ mag01[y & 0x1];
