...
void Sprite::DrawScaled(Rectangle dest, Vector2 origin, float rotation,
                        Color tint) {
...
void Tween::Update() {
    ...
    case MOVE_NORMAL: {
        ...
        //  See if we've overshot; if a sign has changed we've overshot on that
        //  dimension. Just pop to the destination.
        if (newDir.x * dir.x < 0) {
            m_Pos.x = m_Dest.x;
    ...
    case MOVE_BOUNCE: {
        ...
        if (m_Pos.x < m_Dest.x &&
                newPos.x > m_Dest.x ||  //  Overshot going right
            m_Pos.x > m_Dest.x &&
                newPos.x < m_Dest.x ||  //  Overshot going left
            m_Pos.y < m_Dest.y &&
                newPos.y > m_Dest.y ||                  //  Overshot going down
    ...
    case MOVE_PARABOLIC: {
            ...
            m_Pos.y > m_Dest.y && newPos.y < m_Dest.y)  //  Overshot going up
        {
            if (m_OvershootCount == 0) {
                m_Pos = newPos;
                //  Greatly increase the acceleration to pull the item back
...
void MobileSprite::Init(Sprite sprite, Vector2 start, Vector2 dest, float speed,
                        float acceleration) {
...
void Animation::Play(std::string anim, int starttime, int elapsedtime) {
    if (anim == m_CurrentAnim && m_Anims[m_CurrentAnim].m_Looping == true &&
        elapsedtime == 0) {
...
//  For this version to work, all the frames must be on the same horizontal line
//  in the texture.
void Animation::AddFrames(Texture* texture, int posx, int posy, int width,
                          int height, int numFrames) {
    for (int i = 0; i < numFrames; ++i) {
        m_Frames.push_back(make_shared<Sprite>(texture, posx + i * width, posy,
                                               width, height));
    }
}

void Animation::AddFramesSpaced(Texture* texture, int posx, int posy, int width,
                                int height, int numFrames, int xstep) {
    for (int i = 0; i < numFrames; ++i) {
        m_Frames.push_back(make_shared<Sprite>(texture, posx + i * xstep, posy,
                                               width, height));
    }
}

void Animation::AddAnim(std::string name, std::initializer_list<int> frames,
...
void Animation::AddAnim(std::string name, std::vector<int> frames,
                        int frameRate, bool looping) {
    Anim temp(frames, frameRate, looping);
    temp.m_MSPerFrame = 1000 / frameRate;
    m_Anims[name] = temp;
...
Vertex CreateVertex(float inx, float iny, float inz, float inr, float ing,
...
Vertex CreateVertex(float inx, float iny, float inz, Color color, float inu,
                    float inv) {
    Vertex out;
    out.x = inx;
    out.y = iny;
...
Vertex2D CreateVertex2D(float inx, float iny, float inr, float ing, float inb,
                        float ina, float inu, float inv) {
    Vertex2D out;
    out.x = inx;
    out.y = iny;
    out.r = inr;
...
Vertex2D CreateVertex2D(float inx, float iny, Color color, float inu,
                        float inv) {
    Vertex2D out;
    out.x = inx;
    out.y = iny;
...
void SpriteSheet::DrawSprite(int spritex, int spritey, int posx, int posy) {
    Sprite temp(m_Texture, spritex * m_ColumnWidth, spritey * m_RowHeight,
...
void ModTexture::ResizeImage(float newWidth, float newHeight) {
    if (newWidth < 1 || newHeight < 1 || newWidth > width ||
        newHeight > height) {
        return;
    }
