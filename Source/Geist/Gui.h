...
class Gui {
 public:
    ...
    void SetAcceptingInput(bool acceptingInput) {
        m_AcceptingInput = acceptingInput;
    }
    ...
    }  // For most elements, all you need to know is the ID of the most recently
       // active element.
    std::shared_ptr<GuiElement> GetActiveElement();
    std::shared_ptr<GuiElement> GetElement(
        int ID);  //  Use this to get the complete state of a specific element.
    ...
    void AddTextButton(int ID, int posx, int posy, int width, int height,
    ...
    void AddIconButton(int ID, int posx, int posy,
                       std::shared_ptr<Sprite> upbutton,
    ...
    void AddIconButton(int ID, Texture* tex, int posx, int posy, int tilex,
                       int tiley, int width, int height, std::string text = "",
                       Font* font = NULL,
                       Color fontcolor = (Color{255, 255, 255, 255}),
                       int group = 0, int active = true);

    void AddCheckBox(int ID, int posx, int posy,
                     std::shared_ptr<Sprite> Unselected,
                     std::shared_ptr<Sprite> Selected,
                     std::shared_ptr<Sprite> Hovered = NULL,
                     std::shared_ptr<Sprite> HoveredSelected = NULL,
    ...
    void AddRadioButton(int ID, int posx, int posy,
                        std::shared_ptr<Sprite> Selected,
                        std::shared_ptr<Sprite> Unselected = NULL,
                        std::shared_ptr<Sprite> Hovered = NULL,
                        float scalex = 1.0f, float scaley = 1.0f,
                        Color color = Color{255, 255, 255, 255}, int group = 0,
    ...
    void AddScrollBar(int ID, int valuerange, int posx, int posy, int width,
                      int height, bool vertical,
                      Color spurcolor = Color{255, 255, 255, 255},
                      Color backgroundcolor = Color{128, 128, 255, 255},
    ...
    void AddScrollBar(int ID, int valuerange, int posx, int posy, int width,
                      ...
                      std::shared_ptr<Sprite> spurInactive = nullptr,
                      int group = 0, int active = true, bool shadowed = false);

    void AddTextInput(int ID, int posx, int posy, int width, int height,
                      Font* font, std::string initialtext,
                      Color textcolor = Color{255, 255, 255, 255},
    ...
    void AddTextArea(int ID, Font* font, std::string text, int posx, int posy,
                     int width = 0, int height = 0,
                     Color color = Color{255, 255, 255, 255},
                     int justified = GuiTextArea::LEFT, int group = 0,
                     int active = true, bool shadowed = false);

    std::shared_ptr<GuiSprite>
    AddSprite(int ID, int posx, int posy, std::shared_ptr<Sprite> sprite,
              float scalex = 1.0f, float scaley = 1.0f,
              Color color = Color{255, 255, 255, 255}, int group = 0,
              int active = true);

    void AddOctagonBox(int ID, int posx, int posy, int width, int height,
                       std::vector<std::shared_ptr<Sprite>> borders,
                       Color color = Color{255, 255, 255, 255}, int group = 0,
                       int active = true);

    void AddStretchButton(int ID, int posx, int posy, int width,
                          std::string label, std::shared_ptr<Sprite> activeLeft,
                          std::shared_ptr<Sprite> activeRight,
                          std::shared_ptr<Sprite> activeCenter,
    ...
    float m_InputScale;  //  If the gui is scaled, this is the scale factor for
