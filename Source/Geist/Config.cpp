...
void Config::Save() { Save(m_FileName); }

void Config::Save(string filename)
{
	if (filename != m_FileName) m_FileName = filename;
...
{
	if (m_Config.find(node) != m_Config.end() &&
		m_Config.find(node)->second.datatype == DATA_NUMBER)
	{
...
{
	if (m_Config.find(node) != m_Config.end() &&
		m_Config.find(node)->second.datatype == DATA_STRING)
	{
