...
void ShapeEditorState::Update() {
    ...
    if (IsKeyPressed(KEY_A) ||
        m_currentGui->GetActiveElementID() == GE_PREVSHAPEBUTTON) {
        ...
        if (g_shapeTable[newShape][0].IsValid()) {
            ...
            SwitchToGuiForDrawType(
                g_shapeTable[m_currentShape][m_currentFrame].m_drawType);
        } else {
            m_currentShape = 0;
        }
    ...
    if (IsKeyPressed(KEY_D) ||
        m_currentGui->GetActiveElementID() == GE_NEXTSHAPEBUTTON) {
        int newShape = m_currentShape + 1;
    ...
    if (IsKeyPressed(KEY_W) ||
        m_currentGui->GetActiveElementID() == GE_NEXTFRAMEBUTTON) {
    ...
    if (IsKeyPressed(KEY_S) ||
        m_currentGui->GetActiveElementID() == GE_PREVFRAMEBUTTON) {
        int newFrame = m_currentFrame - 1;
        if (newFrame < 0) {
            newFrame = 31;
        }
    ...
    if (m_currentGui->GetActiveElementID() == GE_PREVDRAWTYPE) {
        shapeData.m_drawType = static_cast<ShapeDrawType>(
            static_cast<int>(shapeData.m_drawType) - 1);
        if (shapeData.m_drawType < ShapeDrawType::OBJECT_DRAW_BILLBOARD) {
            shapeData.m_drawType =
                ShapeDrawType(int(ShapeDrawType::OBJECT_DRAW_LAST) - 1);
        }
        SwitchToGuiForDrawType(shapeData.m_drawType);
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_TOPXMINUSBUTTON) {
        if (shapeData.m_topTextureOffsetX + shapeData.m_topTextureWidth - 1 >=
            0) {
            somethingChanged = true;
    ...
    if (m_currentGui->GetActiveElementID() == GE_RIGHTXMINUSBUTTON) {
        if (shapeData.m_rightTextureOffsetX + shapeData.m_rightTextureWidth -
                1 >=
            0) {
            somethingChanged = true;
    ...
    if (m_currentGui->GetActiveElementID() == GE_RIGHTYMINUSBUTTON) {
        if (shapeData.m_rightTextureOffsetY + shapeData.m_rightTextureHeight -
                1 >=
            0) {
            somethingChanged = true;
    ...
    if (m_currentGui->GetActiveElementID() == GE_PREVTOPBUTTON) {
        somethingChanged = true;
        int sideTexture = static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_TOP));
        sideTexture--;
        if (sideTexture < 0) {
            sideTexture = 0;
        }
        shapeData.SetTextureForSide(CuboidSides::CUBOID_TOP,
                                    static_cast<CuboidTexture>(sideTexture));
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_PREVFRONTBUTTON) {
        somethingChanged = true;
        int sideTexture = static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_FRONT));
        sideTexture--;
        if (sideTexture < 0) {
            sideTexture = 0;
        }
        shapeData.SetTextureForSide(CuboidSides::CUBOID_FRONT,
                                    static_cast<CuboidTexture>(sideTexture));
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_PREVRIGHTBUTTON) {
        somethingChanged = true;
        int sideTexture = static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_RIGHT));
        sideTexture--;
        if (sideTexture < 0) {
            sideTexture = 0;
        }
        shapeData.SetTextureForSide(CuboidSides::CUBOID_RIGHT,
                                    static_cast<CuboidTexture>(sideTexture));
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_PREVBOTTOMBUTTON) {
        somethingChanged = true;
        int sideTexture = static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_BOTTOM));
        sideTexture--;
        if (sideTexture < 0) {
            sideTexture = 0;
        }
        shapeData.SetTextureForSide(CuboidSides::CUBOID_BOTTOM,
                                    static_cast<CuboidTexture>(sideTexture));
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_PREVBACKBUTTON) {
        somethingChanged = true;
        int sideTexture = static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_BACK));
        sideTexture--;
        if (sideTexture < 0) {
            sideTexture = 0;
        }
        shapeData.SetTextureForSide(CuboidSides::CUBOID_BACK,
                                    static_cast<CuboidTexture>(sideTexture));
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_PREVLEFTBUTTON) {
        somethingChanged = true;
        int sideTexture = static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_LEFT));
        sideTexture--;
        if (sideTexture < 0) {
            sideTexture = 0;
        }
        shapeData.SetTextureForSide(CuboidSides::CUBOID_LEFT,
                                    static_cast<CuboidTexture>(sideTexture));
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_TWEAKDEPTHPLUSBUTTON) {
        somethingChanged = true;
        shapeData.m_Scaling.z -= .1f;
        if (shapeData.m_Scaling.z < -9.9)
            shapeData.m_Scaling.z = -9.9;
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_TWEAKXPLUSBUTTON) {
        ...
        if (shapeData.m_TweakPos.x < -9.9)
            shapeData.m_TweakPos.x = -9.9;
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_TWEAKYMINUSBUTTON) {
        ...
        if (shapeData.m_TweakPos.y > 9.9)
            shapeData.m_TweakPos.y = 9.9;
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_TWEAKZMINUSBUTTON) {
        ...
        if (shapeData.m_TweakPos.z > 9.9)
            shapeData.m_TweakPos.z = 9.9;
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_USESHAPEPOINTERCHECKBOX) {
        somethingChanged = true;
        shapeData.m_useShapePointer =
            m_currentGui->GetActiveElement()->m_Selected;
    ...
    if (m_currentGui->GetActiveElementID() == GE_TWEAKROTATIONPLUSBUTTON) {
        ...
        if (shapeData.m_rotation > 360)
            shapeData.m_rotation = 0;
        if (shapeData.m_rotation < 0)
            shapeData.m_rotation = 360;
    }
    ...
    if (m_currentGui->GetActiveElementID() == GE_NEXTMODELBUTTON) {
        ...
        g_shapeTable[m_currentShape][m_currentFrame].m_customMeshName =
            (*m_modelIndex).first;
    }
    ...
    if (IsKeyPressed(KEY_S) ||
        m_currentGui->GetActiveElementID() == GE_PREVFRAMEPOINTERBUTTON) {
        int newFrame = shapeData.m_pointerFrame - 1;
        if (newFrame < 0) {
            newFrame = 31;
        }
    ...
    if (g_CameraMoved) {
        ...
        camPos = Vector3RotateByAxisAngle(camPos, Vector3{0, 1, 0},
                                          g_cameraRotation);
    ...
    if (m_currentGui == m_cuboidGui.get()) {
        ...
        m_currentGui->GetElement(GE_TOPSIDETEXTURETEXTAREA)->m_String =
            m_sideDrawStrings[static_cast<int>(
                shapeData.GetTextureForSide(CuboidSides::CUBOID_TOP))];
    ...
    m_currentGui->GetElement(GE_TWEAKWIDTHTEXTAREA)->m_String =
        "W:" + out.str();

    out.str("");
    out.precision(1);
    out << std::fixed << shapeData.m_Scaling.y;
    m_currentGui->GetElement(GE_TWEAKHEIGHTTEXTAREA)->m_String =
        "H:" + out.str();

    out.str("");
    out.precision(1);
    out << std::fixed << shapeData.m_Scaling.z;
    m_currentGui->GetElement(GE_TWEAKDEPTHTEXTAREA)->m_String =
        "D:" + out.str();
    ...
    if (m_currentGui == m_meshGui.get()) {
        m_currentGui->GetElement(GE_MESHOUTLINECHECKBOX)->m_Selected =
            shapeData.m_meshOutline;
    }

    if (m_currentGui == m_meshGui.get()) {
        std::string newStr = g_shapeTable[m_currentShape][m_currentFrame]
                                 .m_customMeshName.substr(16);
        m_currentGui->GetElement(GE_MODELNAMETEXTAREA)->m_String = newStr;
    }

    m_currentGui->GetElement(GE_USESHAPEPOINTERCHECKBOX)->m_Selected =
        shapeData.m_useShapePointer;
}
...
void ShapeEditorState::Draw() {
    ...
    if (g_shapeTable[m_currentShape][m_currentFrame].GetDrawType() ==
        ...
        if (g_shapeTable[m_currentShape][m_currentFrame].m_useShapePointer) {
            d = g_shapeTable[g_shapeTable[m_currentShape][m_currentFrame]
                                 .m_pointerShape]
                            [g_shapeTable[m_currentShape][m_currentFrame]
                                 .m_pointerFrame]
                                .GetTexture();
        }

        Texture* t =
            g_shapeTable[m_currentShape][m_currentFrame].GetTopTexture();
        Texture* f =
            g_shapeTable[m_currentShape][m_currentFrame].GetFrontTexture();
        Texture* r =
            g_shapeTable[m_currentShape][m_currentFrame].GetRightTexture();

        //  Draw original texture with label and border
        DrawTextEx(*g_Font.get(), "Original Texture", {0, 0}, g_fontSize, 1,
                   WHITE);
        float yoffset = g_fontSize + 2;
        DrawRectangleLinesEx({0, yoffset,
                              float(d->width) * scale + scale + scale,
                              float(d->height) * scale + scale + scale},
                             scale, WHITE);
        yoffset += scale;
        DrawTextureEx(*d, Vector2{scale, yoffset}, 0, scale,
                      Color{255, 255, 255, 255});

        //  Draw top texture with labels and borders
        yoffset += float(d->height) * scale + scale + scale;
        float rightoffset = yoffset;
        DrawTextEx(*g_Font.get(), "Top Texture", {0, yoffset}, g_fontSize, 1,
                   WHITE);
        ...
        yoffset += float(d->height) * scale + scale + scale;
        DrawTextEx(*g_Font.get(), "Front Texture", {0, yoffset}, g_fontSize, 1,
                   WHITE);
        yoffset += g_fontSize + 2 + scale;
        DrawTextureEx(*f, Vector2{scale, yoffset}, 0, scale,
                      Color{255, 255, 255, 255});
        yoffset -= scale;
        DrawRectangleLinesEx({0, yoffset,
        ...
        DrawTextEx(*g_Font.get(), "Right Texture", {xoffset, yoffset},
                   g_fontSize, 1, WHITE);
        yoffset += g_fontSize + 2 + scale;
        DrawTextureEx(*r, Vector2{xoffset + scale, yoffset}, 0, scale,
                      Color{255, 255, 255, 255});
        yoffset -= scale;
        DrawRectangleLinesEx({xoffset, yoffset,
    ...
    Vector3 cuboidScaling =
        g_shapeTable[m_currentShape][m_currentFrame].m_Scaling;
    cuboidScaling.x *= 2.5;
    cuboidScaling.y *= 2.5;
    cuboidScaling.z *= 2.5;

    Vector3 finalPos = Vector3Add(
        Vector3Add(g_camera.target,
                   g_shapeTable[m_currentShape][m_currentFrame].m_TweakPos),
        Vector3{g_shapeTable[m_currentShape][m_currentFrame].m_Dims.x / 2 - 1,
                0,
...
void ShapeEditorState::SetupBboardGui() {
    ...
    m_bboardGui->SetLayout(GetRenderWidth() - 120 * g_DrawScale, 0, 120, 360,
                           g_DrawScale, Gui::GUIP_UPPERRIGHT);
...
void ShapeEditorState::SetupFlatGui() {
    ...
    m_flatGui->SetLayout(GetRenderWidth() - 120 * g_DrawScale, 0, 120, 360,
                         g_DrawScale, Gui::GUIP_UPPERRIGHT);
...
void ShapeEditorState::SetupCuboidGui() {
    ...
    m_cuboidGui->SetLayout(GetRenderWidth() - 120 * g_DrawScale, 0, 120, 360,
                           g_DrawScale, Gui::GUIP_UPPERRIGHT);
    ...
    m_cuboidGui->AddTextArea(GE_TOPXTEXTAREA, g_Font.get(),
                             "X: " + to_string(shapeData.m_topTextureOffsetX),
                             14, y);
    ...
    m_cuboidGui->AddTextArea(GE_TOPYTEXTAREA, g_Font.get(),
                             "Y: " + to_string(shapeData.m_topTextureOffsetY),
                             55, y);
    ...
    m_cuboidGui->AddTextArea(GE_TOPWIDTHTEXTAREA, g_Font.get(),
                             "W: " + to_string(shapeData.m_topTextureWidth), 14,
                             y);
    ...
    m_cuboidGui->AddTextArea(GE_TOPHEIGHTTEXTAREA, g_Font.get(),
                             "H: " + to_string(shapeData.m_topTextureHeight),
                             55, y);

    y += yoffset;

    m_cuboidGui->AddTextArea(GE_FRONTTEXTAREA, g_Font.get(), "Front Face", 3,
                             y);
    ...
    m_cuboidGui->AddTextArea(GE_FRONTXTEXTAREA, g_Font.get(),
                             "X: " + to_string(shapeData.m_topTextureOffsetX),
                             14, y);
    ...
    m_cuboidGui->AddTextArea(GE_FRONTYTEXTAREA, g_Font.get(),
                             "Y: " + to_string(shapeData.m_topTextureOffsetY),
                             55, y);
    ...
    m_cuboidGui->AddTextArea(GE_FRONTWIDTHTEXTAREA, g_Font.get(),
                             "W: " + to_string(shapeData.m_topTextureWidth), 14,
                             y);
    ...
    m_cuboidGui->AddTextArea(GE_FRONTHEIGHTTEXTAREA, g_Font.get(),
                             "H: " + to_string(shapeData.m_topTextureHeight),
                             55, y);

    y += yoffset;

    m_cuboidGui->AddTextArea(GE_RIGHTTEXTAREA, g_Font.get(), "Right Face", 3,
                             y);
    ...
    m_cuboidGui->AddTextArea(GE_RIGHTXTEXTAREA, g_Font.get(),
                             "X: " + to_string(shapeData.m_topTextureOffsetX),
                             14, y);
    ...
    m_cuboidGui->AddTextArea(GE_RIGHTYTEXTAREA, g_Font.get(),
                             "Y: " + to_string(shapeData.m_topTextureOffsetY),
                             55, y);
    ...
    m_cuboidGui->AddTextArea(GE_RIGHTWIDTHTEXTAREA, g_Font.get(),
                             "W: " + to_string(shapeData.m_topTextureWidth), 14,
                             y);
    ...
    m_cuboidGui->AddTextArea(GE_RIGHTHEIGHTTEXTAREA, g_Font.get(),
                             "H: " + to_string(shapeData.m_topTextureHeight),
                             55, y);

    y += yoffset;

    m_cuboidGui->AddTextArea(GE_TEXTUREASSIGNMENTTEXTAREA, g_Font.get(),
                             "Texture Assignment:", 2, y);
    ...
    m_cuboidGui->AddTextArea(
        GE_TOPSIDETEXTURETEXTAREA, g_Font.get(),
        m_sideDrawStrings[static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_TOP))],
        50, y);
    ...
    m_cuboidGui->AddTextArea(
        GE_FRONTSIDETEXTURETEXTAREA, g_Font.get(),
        m_sideDrawStrings[static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_FRONT))],
        50, y);
    ...
    m_cuboidGui->AddTextArea(
        GE_RIGHTSIDETEXTURETEXTAREA, g_Font.get(),
        m_sideDrawStrings[static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_RIGHT))],
        50, y);
    ...
    m_cuboidGui->AddTextArea(GE_BOTTOMSIDETEXTAREA, g_Font.get(), "Bottom", 3,
                             y);
    m_cuboidGui->AddIconButton(GE_PREVBOTTOMBUTTON, 40, y, g_LeftArrow);
    m_cuboidGui->AddTextArea(
        GE_BOTTOMSIDETEXTURETEXTAREA, g_Font.get(),
        m_sideDrawStrings[static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_BOTTOM))],
        50, y);
    ...
    m_cuboidGui->AddTextArea(
        GE_BACKSIDETEXTURETEXTAREA, g_Font.get(),
        m_sideDrawStrings[static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_BACK))],
        50, y);
    ...
    m_cuboidGui->AddTextArea(
        GE_LEFTSIDETEXTURETEXTAREA, g_Font.get(),
        m_sideDrawStrings[static_cast<int>(
            shapeData.GetTextureForSide(CuboidSides::CUBOID_LEFT))],
        50, y);
...
void ShapeEditorState::SetupMeshGui() {
    ...
    m_meshGui->SetLayout(GetRenderWidth() - 120 * g_DrawScale, 0, 120, 360,
                         g_DrawScale, Gui::GUIP_UPPERRIGHT);
    ...
    m_meshGui->AddTextArea(GE_MODELNAMETEXTAREA, g_Font.get(),
                           shapeData.m_customMeshName, 12, y);
    m_meshGui->AddIconButton(GE_NEXTMODELBUTTON, 88, y, g_RightArrow);

    y += yoffset;

    m_meshGui->AddCheckBox(GE_MESHOUTLINECHECKBOX, 4, y, 16, 16);
    m_meshGui->GetElement(GE_MESHOUTLINECHECKBOX)->m_Selected =
        shapeData.m_meshOutline;
    m_meshGui->AddTextArea(GE_MESHOUTLINETEXTAREA, g_Font.get(),
                           "Use Mesh Outline", 22, y);
}
...
void ShapeEditorState::SetupCharacterGui() {
    ...
    m_characterGui->SetLayout(GetRenderWidth() - 120 * g_DrawScale, 0, 120, 360,
                              g_DrawScale, Gui::GUIP_UPPERRIGHT);
...
void ShapeEditorState::SetupCommonGui(Gui* gui) {
    ...
    gui->AddTextArea(GE_CURRENTSHAPEIDTEXTAREA, g_Font.get(),
                     "S:" + to_string(m_currentShape), 12, y);

    gui->AddIconButton(GE_PREVFRAMEBUTTON, 55, y, g_LeftArrow);
    gui->AddIconButton(GE_NEXTFRAMEBUTTON, 90, y, g_RightArrow);
    gui->AddTextArea(GE_CURRENTFRAMEIDTEXTAREA, g_Font.get(),
                     "F:" + to_string(m_currentFrame), 65, y);
    ...
    gui->AddTextArea(GE_USESHAPEPOINTERTEXTAREA, g_Font.get(),
                     "Use Shape Pointer", 20, y);

    y += yoffset;

    gui->AddIconButton(GE_PREVSHAPEPOINTERBUTTON, 4, y, g_LeftArrow);
    gui->AddIconButton(GE_NEXTSHAPEPOINTERBUTTON, 45, y, g_RightArrow);
    gui->AddTextArea(GE_CURRENTSHAPEPOINTERIDTEXTAREA, g_Font.get(),
                     "PS: " + to_string(m_currentShape), 15, y);
    ...
    gui->AddTextArea(GE_CURRENTFRAMEPOINTERIDTEXTAREA, g_Font.get(),
                     "PF: " + to_string(m_currentFrame), 65, y);

    y += yoffset;

    gui->AddTextButton(GE_COPYPARAMSFROMFRAME0, 8, y, "Copy From Frame 0",
                       g_Font.get());

    y += yoffset;
    gui->AddTextArea(GE_TWEAKPOSITIONTEXTAREA, g_Font.get(), "Tweak Pos: ", 2,
                     y);
    gui->AddTextArea(GE_TWEAKDIMENSIONSTEXTAREA, g_Font.get(),
                     "Tweak Dims: ", 62, y);
    ...
    out.str("");
    out.precision(1);
    gui->AddTextArea(GE_TWEAKROTATIONTITLEAREA, g_Font.get(), "Tweak Rot:", 2,
                     y);
    gui->AddIconButton(GE_TWEAKROTATIONPLUSBUTTON, 62, y, g_LeftArrow,
                       g_LeftArrow, g_LeftArrow, "", g_Font.get(),
                       Color{255, 255, 255, 255}, 0, 1, true);
