...
TitleState::~TitleState() { Shutdown(); }
...
void TitleState::Update() {
    if (GetTime() - m_LastUpdate > GetFrameTime()) {
        ...
        std::sort(m_sortedVisibleObjects.begin(), m_sortedVisibleObjects.end(),
    ...
    camPos =
        Vector3RotateByAxisAngle(camPos, Vector3{0, 1, 0}, g_cameraRotation);
...
void TitleState::Draw() {
    ...
    DrawTextEx(*g_Font, g_version.c_str(),
               Vector2{GetRenderWidth() * .92f, GetRenderHeight() * .94f},
               g_fontSize, 1, WHITE);
    ...
    DrawTexturePro(
        *m_title,
        Rectangle{0, 0, float(m_title->width), float(m_title->height)},
        Rectangle{0, 0, float(m_title->width * g_DrawScale * .5f),
                  float(m_title->height * g_DrawScale * .5f)},
        {0, 0}, 0, WHITE);
...
void TitleState::CreateTitleGUI() {
    ...
    m_TitleGui->SetLayout(0, 0, g_Engine->m_RenderWidth,
                          g_Engine->m_RenderHeight, g_DrawScale,
                          Gui::GUIP_USE_XY);
    m_TitleGui->AddOctagonBox(GUI_TITLE_PANEL2, 220, 180, 200, 160, g_Borders);
    // m_TitleGui->AddTextArea(GUI_TITLE_TITLE, g_Font.get(), "Ultima VII:
    // Revisited", (320 - (MeasureText("Ultima VII: Revisited", g_Font->baseSize
    // * g_DrawScale))) / 2, 20,
    //    (MeasureText("Ultima VII: Revisited", g_Font->baseSize *
    //    g_DrawScale)), 0, Color{255, 255, 255, 255}, true);

    m_TitleGui->AddStretchButtonCentered(
        GUI_TITLE_BUTTON_SINGLE_PLAYER, 190, "Begin", g_ActiveButtonL,
        g_ActiveButtonR, g_ActiveButtonM, g_ActiveButtonL, g_ActiveButtonR,
        g_ActiveButtonM, 0);

    m_TitleGui->AddStretchButtonCentered(
        GUI_TITLE_BUTTON_SHAPE_EDITOR, 220, "Shape Editor", g_ActiveButtonL,
        g_ActiveButtonR, g_ActiveButtonM, g_ActiveButtonL, g_ActiveButtonR,
        g_ActiveButtonM, 0);

    m_TitleGui->AddStretchButtonCentered(
        GUI_TITLE_BUTTON_OBJECT_EDITOR, 250, "Object Editor", g_ActiveButtonL,
        g_ActiveButtonR, g_ActiveButtonM, g_ActiveButtonL, g_ActiveButtonR,
        g_ActiveButtonM, 0);

    m_TitleGui->AddStretchButtonCentered(
        GUI_TITLE_BUTTON_WORLD_EDITOR, 280, "World Editor", g_ActiveButtonL,
        g_ActiveButtonR, g_ActiveButtonM, g_ActiveButtonL, g_ActiveButtonR,
        g_ActiveButtonM, 0);

    m_TitleGui->AddStretchButtonCentered(
        GUI_TITLE_BUTTON_QUIT, 310, "Quit", g_ActiveButtonL, g_ActiveButtonR,
        g_ActiveButtonM, g_ActiveButtonL, g_ActiveButtonR, g_ActiveButtonM, 0);
...
void TitleState::TestUpdate() {
    ...
    if (IsKeyPressed(KEY_A)) {
        modTexture.ResizeImage(modTexture.m_Image.width - 1,
                               modTexture.m_Image.height);
    ...
    if (IsKeyPressed(KEY_W)) {
        modTexture.ResizeImage(modTexture.m_Image.width,
                               modTexture.m_Image.height - 1);
        modTexture.UpdateTexture();
    }
