...
constexpr char* ShapeDrawTypeStrings[5] = {"Bboard", "Cuboid", "Flat", "Mesh",
                                           "Character"};
...
class ShapeData {
 public:
    ...
    void     Draw(const Vector3& pos, float angle,
    ...
    CuboidTexture GetTextureForSide(CuboidSides side) {
        return m_sideTextures[static_cast<int>(side)];
    }
    void SetTextureForMeshFromSideData(CuboidSides side);
    void SetTextureForSide(CuboidSides side, CuboidTexture texture) {
    ...
    std::array<Model, static_cast<int>(CuboidSides::CUBOID_LAST)>
                m_cuboidModels;
