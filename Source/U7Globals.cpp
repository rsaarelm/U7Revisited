...
std::string g_gameStateStrings[] = {"LoadingState",      "TitleState",
                                    "MainState",         "OptionsState",
                                    "ObjectEditorState", "WorldEditorState"};

std::string g_objectDrawTypeStrings[] = {"Billboard", "Cuboid", "Flat",
                                         "Custom Mesh"};

std::string g_objectTypeStrings[] = {"Static", "Creature",  "Weapon",
                                     "Armor",  "Container", "Quest Item",
                                     "Key",    "Item"};

int g_selectedShape = 150;
int g_selectedFrame = 0;

std::unordered_map<int, int[16][16]>
    g_ChunkTypeList;           // The 16x16 tiles for each chunk type
...
//  (which is 90% of the time).  Much faster because it does not require a
//  square root call. Yes, technically it's "Is less than or equals to".
bool IsDistanceLessThan(float startX, float startZ, float endX, float endZ,
                        float range) {
    float dx = startX - endX;
    float dz = startZ - endZ;
    ...
    for (int i = 0; i < 8; ++i) {
        for (int j = 0; j < 8; ++j) {
            vertices.push_back(CreateVertex(
                -.5, 0, 0, 1, 1, 1, 1, (i + 1) * 0.1250, (j + 1) * 0.1250));
            vertices.push_back(CreateVertex(.5, 0, 0, 1, 1, 1, 1, i * 0.1250,
                                            (j + 1) * 0.1250));
            vertices.push_back(CreateVertex(-.5, 1, 0, 1, 1, 1, 1,
                                            (i + 1) * 0.1250, j * 0.1250));
            vertices.push_back(
                CreateVertex(.5, 1, 0, 1, 1, 1, 1, i * 0.1250, j * 0.1250));
        }
    }
...
unsigned int DoCameraMovement() {
    ...
    if (IsKeyDown(KEY_A)) {
        direction =
            Vector3Add(direction, {-GetFrameTime() * frameTimeModifier, 0,
                                   GetFrameTime() * frameTimeModifier});
        g_CameraMoved = true;
    }
    ...
    if (mouseWheel) {
        if (newDistance >
            g_Engine->m_EngineConfig.GetNumber("camera_far_limit")) {
            newDistance =
                g_Engine->m_EngineConfig.GetNumber("camera_far_limit");
        }

        if (newDistance <
            g_Engine->m_EngineConfig.GetNumber("camera_close_limit")) {
            newDistance =
                g_Engine->m_EngineConfig.GetNumber("camera_close_limit");
        }
    ...
    if (IsLeftButtonDownInRect(
            g_Engine->m_ScreenWidth - (g_minimapSize * g_DrawScale), 0,
            g_Engine->m_ScreenWidth, g_minimapSize * g_DrawScale)) {
        float minimapx = float(GetMouseX() - (g_Engine->m_ScreenWidth -
                                              (g_minimapSize * g_DrawScale))) /
                         float(g_minimapSize * g_DrawScale) * 3072;
        float minimapy =
            float(GetMouseY()) / float(g_minimapSize * g_DrawScale) * 3072;
    ...
    if (!g_CameraMoved &&
        (g_CameraMovementSpeed.x != 0 || g_CameraMovementSpeed.z != 0)) {
        g_CameraMovementSpeed =
            Vector3{g_CameraMovementSpeed.x * .75f, g_CameraMovementSpeed.y,
                    g_CameraMovementSpeed.z * .75f};
    ...
    if (g_CameraMoved) {
        ...
        Vector3 finalmovement = Vector3RotateByAxisAngle(
            g_CameraMovementSpeed, Vector3{0, 1, 0}, g_cameraRotation);
...
shared_ptr<U7Object> GetObjectFromID(int unitID) {
    unordered_map<int, shared_ptr<U7Object>>::iterator finder =
        g_ObjectList.find(unitID);
...
vector<shared_ptr<U7Object>> GetAllUnitsWithinRange(float x, float y,
                                                    float range) {
    vector<shared_ptr<U7Object>> _Targets;
    for (auto& unit : g_ObjectList) {
        if (IsDistanceLessThan(x, y, unit.second->m_Pos.x, unit.second->m_Pos.z,
                               range)) {
...
void DrawConsole() {
    ...
    for (node; node != g_ConsoleStrings.end(); ++node) {
        ...
        if (elapsed < 10) {
            DrawTextEx(
                *g_SmallFont, (*node).m_String.c_str(),
                Vector2{shadowOffset,
                        counter * (g_SmallFont->baseSize + 2) + shadowOffset},
                g_SmallFont->baseSize, 1, Color{0, 0, 0, (*node).m_Color.a});
            DrawTextEx(*g_SmallFont, (*node).m_String.c_str(),
                       Vector2{0, float(counter * (g_SmallFont->baseSize + 2))},
                       g_SmallFont->baseSize, 1, (*node).m_Color);
        }
        ++counter;
    }
    ...
    if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
        if (lmblastState == false) {
            ...
            lmblastTime = GetTime();
                   ...
                   MOUSE_LEFT_BUTTON))  // if (IsMouseButtonPressed
    {
