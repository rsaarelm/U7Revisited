...
void MainState::Init(const string& configfile) {
    ...
    m_MinimapArrow =
        g_ResourceManager->GetTexture("Images/minimaparrow.png", false);
    ...
    m_Gui->m_InputScale =
        float(g_Engine->m_RenderHeight) / float(g_Engine->m_ScreenHeight);
    ...
    m_OptionsGui->AddPanel(9999, 0, 0, 250, 320, Color{255, 255, 255, 255},
                           false);
    m_OptionsGui->AddTextArea(1001, g_Font.get(), "", 125, 100, 0, 0,
                              Color{255, 255, 255, 255}, GuiTextArea::CENTERED);
    m_OptionsGui->AddTextButton(1002, 70, 98, "<-", g_Font.get(),
                                Color{255, 255, 255, 255}, Color{0, 0, 0, 192},
                                Color{255, 255, 255, 255});
    m_OptionsGui->AddTextButton(1003, 170, 98, "->", g_Font.get(),
                                Color{255, 255, 255, 255}, Color{0, 0, 0, 192},
                                Color{255, 255, 255, 255});
...
void MainState::Update() {
    ...
    if (GetTime() - m_LastUpdate > GetFrameTime()) {
        ...
        if (m_showObjects) {
            ...
            for (unordered_map<int, shared_ptr<U7Object>>::iterator node =
                     g_ObjectList.begin();
                 node != g_ObjectList.end(); ++node) {
                (*node).second->Update();
                // Vector3 centerPoint = Vector3Add((*node).second->m_Pos,
                // (*node).second.get()->m_shapeData->m_boundingBoxCenterPoint);

                float distance =
                    Vector3Distance((*node).second->m_Pos, g_camera.target);
                distance -= (*node).second->m_Pos.y;
                if (distance < drawRange &&
                    (*node).second->m_Pos.y <= m_heightCutoff) {
                    double distanceFromCamera =
                        Vector3Distance((*node).second->m_Pos,
                                        g_camera.position) -
                        (*node).second->m_Pos.y;
            ...
            std::sort(
                m_sortedVisibleObjects.begin(), m_sortedVisibleObjects.end(),
    ...
    if (WasRMBDoubleClicked()) {
        AddConsoleString("Right Double-clicked at " + to_string(GetMouseX()) +
                         ", " + to_string(GetMouseY()));
    }

    if (WasLMBDoubleClicked()) {
        // AddConsoleString("Left Double-clicked at " + to_string(GetMouseX()) +
        // ", " + to_string(GetMouseY()));

        std::vector<shared_ptr<U7Object>>::reverse_iterator node;

        for (node = m_sortedVisibleObjects.rbegin();
             node != m_sortedVisibleObjects.rend(); ++node) {
    ...
    //  Get terrain hit for highlight mesh
        ...
        for (node = m_sortedVisibleObjects.rbegin();
            ...
            if (picked) {
                ...
                //					if (WasLMBDoubleClicked())
                //					{
                //						AddConsoleString("This should have
                // opened a GUMP.");
                //					}
                //					else
                //{
                //	AddConsoleString("Object is a container, with " +
                // to_string((*node)->m_inventory.size()) + " objects inside.");

                //	for (auto& item : (*node)->m_inventory)
                //	{
                //		auto object = GetObjectFromID(item);
                //		AddConsoleString("Item: " +
                // g_objectTable[object->m_shapeData->m_shape].m_name + " ID: "
                // + to_string(item));
                //	}
                //}
                //}
                // AddConsoleString("Selected Object: " +
                // to_string(g_selectedShape) + " Frame: " +
                // to_string(g_selectedFrame) + " Name: " +
                // g_objectTable[g_selectedShape].m_name);
...
void MainState::OpenGump(int id) {
    ...
    // gumpGui->AddPanel(9999, 0, 0, 220, 150, Color{ 255, 255, 255, 255 },
    // false); gumpGui->AddTextArea(1001, g_SmallFont.get(), "This is a GUMP",
    // 0, 20, 0, g_SmallFont.get()->baseSize * 1.5f, Color{255, 255, 255, 255},
    // GuiTextArea::LEFT); gumpGui->AddTextButton(1002, 70, 98, "<-",
    // g_Font.get(), Color{ 255, 255, 255, 255 }, Color{ 0, 0, 0, 192 }, Color{
    // 255, 255, 255, 255 }); gumpGui->AddTextButton(1003, 10, 100,
    // "CheckMark!", g_SmallFont.get(), Color{ 255, 255, 255, 255 }, Color{ 0,
    // 0, 0, 192 }, Color{ 255, 255, 255, 255 });
    gumpGui->AddSprite(1004, 0, 0, g_gumpBackground, 1.0f, 1.0f,
                       Color{255, 255, 255, 255});
    gumpGui->AddIconButton(1005, 4, 34, g_gumpCheckmarkUp, g_gumpCheckmarkDown,
                           g_gumpCheckmarkUp, "", g_SmallFont.get(),
                           Color{255, 255, 255, 255}, 0, 1, false);
...
void MainState::Draw() {
    ...
    float ratio =
        float(g_Engine->m_ScreenWidth) / float(g_Engine->m_RenderWidth);
    if (g_pixelated) {

        EndTextureMode();
        DrawTexturePro(g_renderTarget.texture,
                       {0, 0, float(g_renderTarget.texture.width),
                        float(g_renderTarget.texture.height)},
    ...
    DrawTexturePro(g_guiRenderTarget.texture,
                   {0, 0, float(g_guiRenderTarget.texture.width),
                    float(g_guiRenderTarget.texture.height)},
                   {0, float(g_Engine->m_ScreenHeight),
                    float(g_Engine->m_ScreenWidth),
                    -float(g_Engine->m_ScreenHeight)},
                   {0, 0}, 0, WHITE);

    // DrawTexturePro(g_guiRenderTarget.texture, { 0, 0,
    // g_Engine->m_RenderWidth, -g_Engine->m_RenderHeight }, { -ratio, -ratio,
    // g_Engine->m_ScreenWidth + (ratio * 2), g_Engine->m_ScreenHeight + (ratio
    // * 2) }, { 0, 0 }, 0, WHITE);

    DrawTextureEx(
        *m_Minimap,
        {g_Engine->m_ScreenWidth - float(g_minimapSize * g_DrawScale), 0}, 0,
        float(g_minimapSize * g_DrawScale) / float(m_Minimap->width), WHITE);
    // DrawTexture(*m_Minimap, g_Engine->m_RenderWidth -
    // float(m_Minimap->width), 0, WHITE);

    float _ScaleX = (g_minimapSize * g_DrawScale) / float(g_Terrain->m_width) *
                    g_camera.target.x;
    float _ScaleZ = (g_minimapSize * g_DrawScale) / float(g_Terrain->m_height) *
                    g_camera.target.z;

    float half = float(g_DrawScale) * float(m_MinimapArrow->width) / 2;

    DrawTextureEx(*m_MinimapArrow,
                  {g_Engine->m_ScreenWidth -
                       float(g_minimapSize * g_DrawScale) + _ScaleX - half,
                   _ScaleZ - half},
                  0, g_DrawScale, WHITE);

    DrawTextureEx(*g_Cursor, {float(GetMouseX()), float(GetMouseY())}, 0,
                  g_DrawScale, WHITE);
