            ...
            if (picked) {
                ...
                if ((*node)->m_isContainer) {
                    AddConsoleString("Object is a container, with " +
                                     to_string((*node)->m_inventory.size()) +
                                     " objects inside.");
                    ...
                    for (auto& item : (*node)->m_inventory) {
                        ...
                        AddConsoleString(
                            "Item: " +
                            g_objectTable[object->m_shapeData->m_shape].m_name +
                            " ID: " + to_string(item));
                    }
                }
                AddConsoleString(
                    "Selected Object: " + to_string(g_selectedShape) +
                    " Frame: " + to_string(g_selectedFrame) +
                    " Name: " + g_objectTable[g_selectedShape].m_name);
...
void MainState::Draw() {
    ...
    if (g_pixelated) {
        ...
        DrawTexturePro(g_renderTarget.texture,
                       ...
                       {0, 0}, 0, WHITE);
        // DrawTexturePro(g_renderTarget.texture, { 0, 0,
        // g_Engine->m_RenderWidth, -g_Engine->m_RenderHeight }, { -ratio,
        // -ratio, g_Engine->m_ScreenWidth + (ratio * 2),
        // g_Engine->m_ScreenHeight + (ratio * 2)}, {0, 0}, 0, WHITE);
    }
