...
void LoadingState::Draw() {
    ...
    if (m_loadingFailed == true) {
        std::string missingDataText =
            "Ultima VII files not found.  They should go into the " +
            g_Engine->m_EngineConfig.GetString("data_path") + " folder.";
        DrawTextEx(*g_Font, missingDataText.c_str(), Vector2{0, 0}, g_fontSize,
                   1, WHITE);
        DrawTextEx(*g_Font, "Press ESC to exit.", Vector2{0, g_fontSize * 2},
                   g_fontSize, 1, WHITE);
    } else {
        DrawConsole();
    }
...
void LoadingState::LoadChunks() {
    ...
    if (u7chunksfile == nullptr) {
        Log("Ultima VII files not found.  They should go into the Data/U7 "
            "folder.");
        m_loadingFailed = true;
        return;
    }

    //  Each chunk should be an ID associated with 16 arrays, each 16 unsigned
    //  chars deep.
...
void LoadingState::LoadIFIX() {
    ...
    for (int superchunky = 0; superchunky < 12; ++superchunky) {
        for (int superchunkx = 0; superchunkx < 12; ++superchunkx) {
            ...
            //  which is the same for every file: "Ultima VII Data File (C) 1992
            //  Origin Inc."
            char header[80];
            fread(&header, sizeof(char), 80, u7thisifix);

            //  This is followed two unsigned ints.  The first unsigned int is
            //  always the same, so we can ignore it. The second is the number
            //  of g_objecTable in the file.
            ...
            //  Now, having processed the header, we can process the, you know,
            //  data.
            for (int chunky = 0; chunky < 16; ++chunky) {
                for (int chunkx = 0; chunkx < 16; ++chunkx) {
                    ...
                    if (thisentry.offest == 0) {
                        ...
                        fseek(u7thisifix, thisentry.offest * sizeof(char),
                              SEEK_SET);
                        fread(locationdata, sizeof(unsigned char),
                              thisentry.length, u7thisifix);
                        ...
                        for (int w = 0; w < (thisentry.length / 2); w += 2) {
                            ...
                            AddObject(shape, frame, GetNextID(),
        ...
        for (int j = 0; j < 192; ++j) {
            ...
            for (int k = 0; k < 16; ++k) {
                for (int l = 0; l < 16; ++l) {
                    ...
                    g_World[j * 16 + k][i * 16 + l] =
                        g_ChunkTypeList[chunkid][l][k];

                    unsigned short shapenum = thisdata & 0x3ff;
                    unsigned short framenum = (thisdata >> 10) & 0x1f;

                    if (shapenum >= 150) {
                        AddObject(shapenum, framenum, GetNextID(), (i * 16 + k),
                                  0, (j * 16 + l));
                    }
                }
            }
    ...
    for (int superchunky = 0; superchunky < 12; ++superchunky) {
        for (int superchunkx = 0; superchunkx < 12; ++superchunkx) {
            ...
            if (u7thisireg == nullptr) {
                Log("Ultima VII files not found.  They should go into the "
                    "Data/U7 folder.");
                m_loadingFailed = true;
                return;
            } else {
                ...
                while (!feof(u7thisireg)) {
                    ...
                    if (length == 6)  //  Object.
                    {
                        ...
                        int actualx =
                            (superchunkx * 256) + (chunkx * 16) + intx;
                        int actualy =
                            (superchunky * 256) + (chunky * 16) + inty;

                        unsigned short shapeData;
                        fread(&shapeData, sizeof(unsigned short), 1,
                              u7thisireg);
                        int           shape = shapeData & 0x3ff;
                        int           frame = (shapeData >> 10) & 0x1f;
                        ...
                        if (shape != 275 && shape != 607 &&
                            shape != 0)  //  Eggs
                        {
                            int objectId = GetNextID();
                            AddObject(shape, frame, objectId, actualx, lift1,
                                      actualy);
                    ...
                    } else if (length == 12)  // Container or Egg
                    {
                        ...
                        unsigned char sink;
                        for (int i = 0; i < 5; ++i) {
                            fread(&sink, sizeof(unsigned char), 1,
                                  u7thisireg);  // 5-9
                        }
                        ...
                        fread(&throwaway, sizeof(unsigned char), 1,
                              u7thisireg);  // 11
                        ...
                        fread(&eggOrContainer, sizeof(unsigned char), 1,
                              u7thisireg);  // 12
                        if (eggOrContainer == 0) {
                            containerOpen = true;
                            containerId = id;
                            // AddObject(shape, frame, containerId, actualx,
                            // lift1, actualy);
                        }
...
void LoadingState::CreateShapeTable() {
    ...
    if (!palette.good()) {
        Log("Ultima VII files not found.  They should go into the "
            "Data/U7/blackgate folder.");
        throw("Ultima VII files not found.  They should go into the "
              "Data/U7/blackgate folder.");
    }
    ...
    unsigned char* paletteData =
        (unsigned char*)malloc(paletteEntryMap[0].length);
    palette.read((char*)paletteData, paletteEntryMap[1].length);

    //  Currently only loading the base palette.  Other palettes are for
    //  lighting effects.
    for (int j = 0; j < 256; ++j) {
        ...
        for (int thisFrame = 0; thisFrame < numFrames; ++thisFrame) {
            ...
            for (int i = 0; i < 8; ++i) {
                for (int j = 0; j < 8; ++j) {
                    ...
                    ImageDrawPixel(&tempImage, (thisShape * 8) + j,
                                   (thisFrame * 8) + i, m_palette[Value]);
                }
            }
        }
    }
    ...
    for (int thisShape = 150; thisShape < 1024; ++thisShape) {
        ...
        //  If the first data is the same as the length of the shape entry, then
        //  this entry is run-length encoded.
        if (firstData == shapeEntryMap[thisShape].length) {
            ...
            for (int i = 0; i < frameCount; ++i) {
                ...
                frameOffsets[i].height =
                    frameOffsets[i].H1 + frameOffsets[i].H2 + 1;
                frameOffsets[i].width =
                    frameOffsets[i].W2 + frameOffsets[i].W1 + 1;
                ...
                Image tempImage =
                    GenImageColor(frameOffsets[i].width, frameOffsets[i].height,
                                  Color{0, 0, 0, 0});
                ...
                while (true) {
                    ...
                    if (blockData == 0) {
                        break;  //  There are no more spans; we're done with
                                //  this frame.
                    }
                    ...
                    xStart +=
                        frameOffsets[i].width - frameOffsets[i].xDrawOffset - 1;
                    yStart += frameOffsets[i].height -
                              frameOffsets[i].yDrawOffset - 1;

                    if (spanType == 0)  // Not RLE, raw pixel data.
                    {
                        for (int i = 0; i < spanLength; ++i) {
                            unsigned char Value = ReadU8(shapes);
                            ImageDrawPixel(&tempImage, xStart + i, yStart,
                                           m_palette[Value]);
                        }
                    } else  // RLE.
                    {
                        ...
                        while (xStart < endX) {
                            ...
                            if (runType == 0)  // Once again, non-RLE
                            {
                                for (int i = 0; i < runLength; ++i) {
                                    unsigned char Value = ReadU8(shapes);
                                    ImageDrawPixel(&tempImage, xStart + i,
                                                   yStart, m_palette[Value]);
                                }
                            } else {
                                unsigned char Value = ReadU8(shapes);
                                for (int i = 0; i < runLength; ++i) {
...
void LoadingState::LoadModels() {
    ...
    while (!directory.eof()) {
        ...
        std::string objPath =
            "Models/3dmodels/" + m_Filename + std::string(".obj");
        std::string mtlPath =
            "Models/3dmodels/" + m_Filename + std::string(".mtl");

        Model     model = LoadModel(objPath.c_str());
        int       materialCount = 0;
        Material* material =
            LoadMaterials(mtlPath.c_str(), &materialCount);  // Load material
...
void LoadingState::CreateObjectTable() {
    ...
    //  This is followed two unsigned ints.  The first unsigned int is always
    //  the same, so we can ignore it. The second is the number of g_objectTable
    //  in the file.
...
std::vector<LoadingState::FLXEntryData>
LoadingState::ParseFLXHeader(istream& file) {
    char header[80];
    file.read(header, sizeof(header));

    //  This is followed two unsigned ints.  The first unsigned int is always
    //  the same, so we can ignore it. The second is the number of g_objecTable
    //  in the file.
    unsigned int throwaway;
    unsigned int entrycount;
    ...
    for (auto& node = subFileMap.begin(); node != subFileMap.end(); ++node) {
        ...
        if (!strncmp(filename, "npc.dat", 6)) {
            ...
            for (int i = 0; i < fullcount; ++i) {
                if (i == 139)  // This NPC is broken and attempting to parse it
                               // messes up all NPCs after it.  It's not used
                               // anyway so we just skip it.
                {
                ...
                AddObject(shapenum, 16, GetNextID(),
                          chunkx * 16 * 16 + thisNPC.x, thisNPC.lift >> 4,
                ...
                Log("Iolo starts at 2761 so there are " +
                    to_string(2761 - newfilepos) + " bytes left.");
